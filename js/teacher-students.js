...
function loadData() {
    ...
    if (Array.isArray(storedStudents) && storedStudents.length) {
        students = storedStudents.map(student => ({
            ...student,
            attendanceRecords: student.attendanceRecords || {}
        }));
    }
    const selectedStudentId = Number(localStorage.getItem('selectedStudentId'));
    if (selectedStudentId) {
        setTimeout(() => {
            viewStudentDetails(selectedStudentId);
    ...
    if (students.length === 0) {
        students = [
            ...
            },
            {
                ...
                grades: [
                    { subject: 'web', value: 4, type: 'exam', date: '2025-09-10', comment: '' },
                    { subject: 'cloud', value: 5, type: 'test', date: '2025-09-12', comment: 'Хорошо' }
                ],
                attendance: 92
            },
            {
                ...
                phone: '+77001234569',
                grades: [
                    { subject: 'lang', value: 3, type: 'exam', date: '2025-09-11', comment: 'Нужно подучить' }
                ],
                attendance: 70
            }
        ];
